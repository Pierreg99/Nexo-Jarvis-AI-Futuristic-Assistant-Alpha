...
function CameraResponse({ state, reducedMotion }: { state: NexoCoreState; reducedMotion: boolean }) {
  ...
  useFrame((_, delta) => {
    if (reducedMotion) {
      perspectiveCamera.position.copy(target);
      perspectiveCamera.fov = state === "thinking" ? 35 : state === "listening" ? 40 : 42;
...
function OrbitRing({ radius, tube, tilt, speed, color, reducedMotion }: { radius: number; tube: number; tilt: [number, number, number]; speed: number; color: string; reducedMotion: boolean }) {
  const ref = useRef<THREE.Mesh>(null);
  useFrame((_, delta) => {
    if (ref.current && !reducedMotion) ref.current.rotation.z += delta * speed;
  });
  return (
...
function NexoMachine({ state, reducedMotion }: { state: NexoCoreState; reducedMotion: boolean }) {
  const palette = statePalette[state];
  const group = useRef<THREE.Group>(null);
  const core = useRef<THREE.Mesh>(null);
  const glow = useRef<THREE.PointLight>(null);
  const material = useMemo(() => new THREE.MeshPhysicalMaterial({
    color: "#07131a", metalness: 0.92, roughness: 0.17, transmission: 0.05, clearcoat: 1, clearcoatRoughness: 0.12,
  }), []);
  ...
  useFrame(({ clock }, delta) => {
    ...
    if (core.current) {
      const breathing = reducedMotion ? 1 : 1 + Math.sin(elapsed * (state === "thinking" ? 5.5 : 2.2)) * (state === "thinking" ? 0.12 : 0.055);
      core.current.scale.setScalar(breathing);
    }
    if (glow.current) glow.current.intensity = reducedMotion ? palette.intensity : palette.intensity + Math.sin(elapsed * 3.1) * 0.35;
  });
  ...
  return (
    <group ref={group}>
      ...
      <OrbitRing radius={1.7} tube={0.024} tilt={[0.85, 0.2, 0.22]} speed={palette.speed * 0.42} color="#26e4ff" reducedMotion={reducedMotion} />
      <OrbitRing radius={1.98} tube={0.016} tilt={[-0.38, 0.68, 0.9]} speed={-palette.speed * 0.34} color="#43c5d8" reducedMotion={reducedMotion} />
      <OrbitRing radius={2.18} tube={0.014} tilt={[0.22, -0.48, -0.44]} speed={palette.speed * 0.23} color={state === "thinking" ? "#ffb14a" : "#116d81"} reducedMotion={reducedMotion} />
      <OrbitRing radius={1.06} tube={0.025} tilt={[1.25, 0, 0.32]} speed={palette.speed * 0.9} color="#8cf5ff" reducedMotion={reducedMotion} />

      <Sparkles count={reducedMotion ? 28 : state === "thinking" ? 130 : 88} scale={[6, 5, 4]} size={reducedMotion ? 1.2 : state === "speaking" ? 2.7 : 1.8} speed={reducedMotion ? 0 : palette.speed * 0.75} color={palette.energy} noise={1.8} />
      <Stars radius={7} depth={3} count={reducedMotion ? 80 : 360} factor={2.3} saturation={0.25} fade speed={reducedMotion ? 0 : 0.4} />
    </group>
  );
}
...
export default function NexoCore3D({ state, immersive = false }: NexoCore3DProps) {
  ...
  return (
    <div className={`nexo-webgl-scene ${immersive ? "nexo-webgl-scene-immersive" : ""}`} role="img" aria-label={`Interactive three-dimensional Nexo core, ${state} state`}>
      <Canvas dpr={[1, immersive ? 2 : 1.6]} camera={{ position: [0, 0.1, immersive ? 6.2 : 6.8], fov: immersive ? 38 : 42 }} gl={{ antialias: true, alpha: true, powerPreference: "high-performance" }}>
        ...
        <NexoMachine state={state} reducedMotion={reducedMotion} />
        <CameraResponse state={state} reducedMotion={reducedMotion} />
        <ContactShadows position={[0, -2.25, 0]} opacity={0.58} scale={8} blur={2.4} far={4.5} color="#001e2a" />
        <OrbitControls enablePan={false} enableDamping={!reducedMotion} dampingFactor={0.08} minDistance={4.4} maxDistance={8.6} minPolarAngle={0.68} maxPolarAngle={2.3} autoRotate={!reducedMotion && state !== "thinking"} autoRotateSpeed={statePalette[state].speed * 0.55} />
      </Canvas>
      <div className="nexo-webgl-vignette" aria-hidden="true" />
      {!reducedMotion && <div className="nexo-webgl-reticle" aria-hidden="true"><span /><span /><span /><span /></div>}
    </div>
  );
}
